"""Train victim GCN model on Pubmed dataset."""
import sys
sys.path.append('.')
...
def main():
    print("Training victim GCN on Pubmed dataset...")
    print("=" * 50)
    
    # Initialize trainer
    trainer = VictimTrainer('Pubmed', hidden_dim=16)
    
    # Train model (60 training samples as per paper)
    history = trainer.train(num_train_per_class=20, epochs=200, verbose=True)
    
    # Save model
    trainer.save_model('models/victim_pubmed.pth')
